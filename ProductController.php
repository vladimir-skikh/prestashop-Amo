    ...
    public function initContent()
    {
        if (!$this->errors) {
            ...
            if ($this->product->customizable) {
                $files = $this->context->cart->getProductCustomization($this->product->id, CUSTOMIZE_FILE, true);
            ...
            //Изменение шаблона в сводке на город из геолокации
            $templateCity = '{$#_city}';
            $cookieCity = $this->context->cookie->pw_city_name;

            $product_for_template['description_short'] = str_replace('{$#_city}', $cookieCity, $product_for_template['description_short']);
